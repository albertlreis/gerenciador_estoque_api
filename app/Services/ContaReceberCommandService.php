...
class ContaReceberCommandService
{
    ...
    public function criar(array $dados): ContaReceber
    {
        if (!empty($dados['recorrencia']) && !empty($dados['parcelamento'])) {
            throw ValidationException::withMessages([
                'recorrencia' => 'Recorrência e parcelamento não podem ser usados no mesmo lançamento.',
            ]);
        }
        ...
        return DB::transaction(function () use ($dados) {
            ...
            unset($dados['pagamento_inicial'], $dados['parcelamento'], $dados['recorrencia']);
    ...
    {
        $antesPagamento = $pagamento->toArray();
        $this->ledger->cancelarLancamentoPorPagamento($pagamento);
    ...
    private function excluirTituloContaAzulBestEffort(int $contaReceberId, string $evento): void
    {
        try {
            DB::afterCommit(function () use ($contaReceberId, $evento): void {
                try {
                    $this->contaAzulExports->excluirTituloFinanceiro(
    ...
    private function criarParcelado(array $dados): ContaReceber
    {
        return DB::transaction(function () use ($dados) {
            ...
            $pagamentoInicial = $dados['pagamento_inicial'] ?? null;
            unset($dados['parcelamento'], $dados['recorrencia'], $dados['pagamento_inicial']);
            ...
            $target = collect($contas)->first(
                fn (ContaReceber $conta) => (bool) $conta->is_entrada
            ) ?? ($contas[0] ?? null);
            ...
            foreach ($contas as $conta) {
                if ($target && is_array($pagamentoInicial) && $conta->is($target)) {
                    continue;
                }
                $this->exportarTituloContaAzulBestEffort((int) $conta->id, 'conta_receber_parcelada_criada');
            }
    ...
    private function parcelasPayload(
        ...
        if (!empty($parcelamentoDados['parcelas']) && is_array($parcelamentoDados['parcelas'])) {
            return $this->parcelasCustomizadasPayload(
                $base,
                $parcelamento,
                $parcelamentoDados,
                $valorTotal,
        ...
        $entradaDate = Carbon::parse(
            $parcelamentoDados['data_entrada'] ?? $base['data_emissao'] ?? now()
        )->toDateString();
        $primeiroVencimento = Carbon::parse(
            $parcelamentoDados['primeiro_vencimento'] ?? $base['data_vencimento'] ?? now()
        );
        $comum = $base;
        ...
        foreach ($valores as $idx => $valor) {
            ...
            $payloads[] = [
                ...
                'data_vencimento' => $primeiroVencimento->copy()
                    ->addMonthsNoOverflow($idx * $intervalo)
                    ->toDateString(),
                'valor_bruto' => $valor,
                'valor_liquido' => $valor,
                'saldo_aberto' => $valor,
    ...
    private function parcelasCustomizadasPayload(
        ...
        $parcelas = collect($parcelamentoDados['parcelas'])
            ->map(function (array $parcela) {
                $isEntrada = (bool) ($parcela['is_entrada'] ?? false)
                    || (int) ($parcela['parcela_numero'] ?? 0) === 0;
            ...
            ->sortBy(fn (array $parcela) => ($parcela['is_entrada'] ? '0' : '1')
                . str_pad((string) $parcela['parcela_numero'], 3, '0', STR_PAD_LEFT))
            ->values();

        if ($this->moneyCents($parcelas->sum('valor')) !== $this->moneyCents($valorTotal)) {
            throw ValidationException::withMessages([
        ...
        if ($this->moneyCents($entradas->sum('valor')) !== $this->moneyCents($valorEntrada)) {
            throw ValidationException::withMessages([
        ...
        return $parcelas->map(function (array $parcela) use ($comum, $descricao, $quantidade) {
            ...
            return [
                ...
                'descricao' => $isEntrada
                    ? "{$descricao} - Entrada"
                    : "{$descricao} - Parcela {$numero}/{$quantidade}",
    ...
    {
        $valor = (float) $this->toDecimal($dados['valor'] ?? 0);
