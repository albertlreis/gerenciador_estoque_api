...
<div class="report-header">
    ...
    <div class="report-meta">
        ...
        <span class="muted">{{ $dataGeracao }}</span>
    </div>
</div>
...
<table>
    ...
    <tr>
        ...
        <th class="date">Recebimento</th>
        <th class="summary">Resumo do lancamento</th>
        <th class="money">Total (R$)</th>
        <th class="money">Recebido (R$)</th>
        <th class="money">A receber (R$)</th>
    ...
    @forelse($contas as $conta)
        @php
            $pagamentos = $conta->pagamentos ?? collect();
            $ultimoPagamento = $pagamentos->sortByDesc('data_pagamento')->first();
            $liquido = (float) ($conta->valor_liquido ?? ((float) $conta->valor_bruto - (float) $conta->desconto + (float) $conta->juros + (float) $conta->multa));
            $recebido = (float) $pagamentos->sum('valor');
            $saldo = max(0, $liquido - $recebido);
            $totalLiquido += $liquido;
            $totalRecebido += $recebido;
            $totalSaldo += $saldo;
            $cliente = $conta->cliente?->nome ?: $conta->pedido?->cliente?->nome;
            $pedido = $conta->pedido?->numero_externo ? 'Pedido ' . $conta->pedido->numero_externo : null;
            $doc = $conta->numero_documento ? 'Doc ' . $conta->numero_documento : null;
            $detalhes = collect([$cliente, $pedido, $doc])->filter()->implode(' | ');
        @endphp
        <tr>
            <td class="date">{{ optional($conta->data_vencimento)->format('d/m/Y') ?: '-' }}</td>
            ...
            <td class="money">{{ number_format($recebido, 2, ',', '.') }}</td>
            <td class="money">{{ number_format($saldo, 2, ',', '.') }}</td>
            <td class="status">{{ $conta->status?->value ?? $conta->status }}</td>
        </tr>
    @empty
        <tr>
    ...
    <tr>
        ...
        <td class="money">{{ number_format($totalRecebido, 2, ',', '.') }}</td>
